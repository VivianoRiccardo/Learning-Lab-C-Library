...
int main(){
    ...
    for(i = 0; i < n_layers-1; i++){
        l[i] = recurrent_lstm(rmodel_size,NO_DROPOUT,0.1,NO_DROPOUT,0.1,i,window,LSTM_RESIDUAL,NO_NORMALIZATION,0,EDGE_POPUP,EDGE_POPUP);
		l[i]->k_percentage = 0.5;
    }
    ...
    fcls[0] = fully_connected(rmodel_size,output_dimension,0,NO_DROPOUT,SOFTMAX,0,0,NO_NORMALIZATION,FREEZE_TRAINING,FULLY_FEED_FORWARD);
    ...
    for(i = 0; i < batch_size; i++){
        ...
        batch_r[i] = copy_rmodel_without_learning_parameters(r);
    }
    ...
    for(k = 0; k < epochs; k++){
        ...
        for(i = 0; i < training_instances/batch_size; i++){
            ...
            ff_rmodel_lstm_multicore_opt(h,c,&inputs_rmodel[i*batch_size],batch_r,batch_size,threads,r);
            for(j = 0; j < batch_size; j++){
                copy_array(batch_r[j]->lstms[n_layers-1]->out_up[window-1],outputs_rmodel[j],rmodel_size);
            }
            ff_error_bp_model_multicore_opt(batch_m,m,rmodel_size,1,1,outputs_rmodel,batch_size,threads,&outputs[i*batch_size],ret_model_error);
            for(j = 0; j < batch_size; j++){
                copy_array(ret_model_error[j],error_lstms[j][window-1],rmodel_size);
            }
            bp_rmodel_lstm_multicore_opt(h,c,&inputs_rmodel[i*batch_size],batch_r,error_lstms,batch_size,threads,dfioc, NULL,r);
            for(j = 0; j < batch_size; j++){
            ...
            for(j = 0; j < batch_size; j++){
                ...
                reset_rmodel_without_learning_parameters(batch_r[j]);
    ...
    for(i = 0; i < batch_size; i++){
        ...
        free_rmodel_without_learning_parameters(batch_r[i]);
        free(errors[i]);
    }
    free(batch_r);
    free(errors);
